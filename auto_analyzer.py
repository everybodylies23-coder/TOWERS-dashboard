...
<body class="min-h-screen p-4 md:p-8">
    ...
    <script>
        ...
        function updateDashboardForDate(targetDate) {{
            ...
            // 2. Load AI Summary text (Markdown render & strip ④, ⑤, ⑥ to prevent duplication!)
            const summaryObj = rawSummaries.find(s => s.date === targetDate);
            if (summaryObj && summaryObj.text) {{
                let cleanText = summaryObj.text;
                const cutoffIndex = cleanText.search(/(④|4\.|⑤|5\.|⑥|6\.|AI独自の次回推奨狙い台)/);
                if (cutoffIndex !== -1) {{
                    cleanText = cleanText.substring(0, cutoffIndex).trim();
                }}
